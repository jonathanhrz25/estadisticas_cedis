...
// Conexión utilizando MySQLi para la tabla merida
$conn = connectMysqli();
...
foreach ($tables as $table) {
    try {
        ...
        if (in_array('especificaciones', $columns)) {
            $sql = "SELECT Id, clave, area, equipo, marca, modelo, especificaciones FROM $table WHERE clave LIKE 'MR%'";
        } else {
            // Si no existe, omitirla
            $sql = "SELECT Id, clave, area, equipo, marca, modelo FROM $table WHERE clave LIKE 'MR%'";
        }
        ...
        // Insertar o actualizar los datos en la tabla merida
        foreach ($result as $row) {
            ...
            // Verificar si la clave ya existe en la tabla merida
            $sql_check = "SELECT Id FROM merida WHERE clave = '$clave'";
            $result_check = mysqli_query($conn, $sql_check);

            if (mysqli_num_rows($result_check) > 0) {
                // Si existe, actualizar el registro
                $sql_update = "UPDATE merida SET 
                    area = '$area',
                    equipo = '$equipo',
                    marca = '$marca',
                    modelo = '$modelo'" .
                ...
                if (!mysqli_query($conn, $sql_update)) {
                    echo "Error al actualizar el registro en merida para la tabla $table: " . mysqli_error($conn);
                }
            } else {
                // Si no existe, insertar un nuevo registro
                $sql_insert = "INSERT INTO merida (clave, area, equipo, marca, modelo" .
                    ($especificaciones !== null ? ", especificaciones" : "") .
                    ") VALUES ('$clave', '$area', '$equipo', '$marca', '$modelo'" .
                    ($especificaciones !== null ? ", '$especificaciones'" : "") . ")";
                    
                if (!mysqli_query($conn, $sql_insert)) {
                    echo "Error al insertar el registro en merida para la tabla $table: " . mysqli_error($conn);
                }
            }
        }
...
echo "Datos guardados exitosamente en la tabla merida.";
?>
        ...
        <header>
            <nav class="navbar navbar-dark bg-dark fixed-top" style="background-color: #081856!important;">
                <div class="container-fluid">
                    ...
                    <a class="btn btn-success ml-2" href="../formulario/form_merida.php">Añadir</a>
                </div>
            </nav>
        </header>

        <div style="padding-top: 40px;"></div>
        <h1 class="display-6" style="color:black">Cedis Merida</h1>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    $result_pc = $conn->query("SELECT * FROM merida WHERE clave LIKE 'MR%'");
                    if ($result_pc->num_rows > 0) {
                        while ($mostrar_pc = $result_pc->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td class="text-center icon-cell"><a href="../modificar/modificarMerida.php?id=<?php echo $mostrar_pc['Id']; ?>"><i class="fa fa-pencil-square-o fa-2x"></i></a></td>
                            </tr>
                            <?php
                        }
                    } else {
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    $result_camaras = $conn->query("SELECT * FROM merida WHERE clave LIKE 'MR%'");
                    if ($result_camaras->num_rows > 0) {
                        while ($mostrar_camaras = $result_camaras->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td class="text-center icon-cell"><a href="../modificar/modificarMerida2.php?id=<?php echo $mostrar_camaras['Id']; ?>"><i class="fa fa-pencil-square-o fa-2x"></i></a></td>
                            </tr>
                            <?php
                        }
                    } else {
